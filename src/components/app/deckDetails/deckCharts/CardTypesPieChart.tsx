...
const initialColorDistributionChartData = [
  { type: "character", types: 0, fill: "var(--amber)" },
  { type: "item", types: 0, fill: "var(--ruby)" },
  { type: "action", types: 0, fill: "var(--sapphire)" },
  { type: "location", types: 0, fill: "var(--steel)" },
];

export function CardTypesPieChart({ data }: ChartProps) {
  const chartData =
    data.reduce(
      (acc, { lorcanitoCard, qty }) => {
        const cardType = lorcanitoCard.type;
